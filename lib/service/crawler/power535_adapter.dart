...
class Power535Adapter implements BaseCrawlerAdapter {
  ...
  Future<List<LotteryDrawModel>> fetchPage(int pageIndex) async {
    final url = Uri.parse('https://vietlott.vn/ajaxpro/Vietlott.PlugIn.WebParts.Game535CompareWebPart,Vietlott.PlugIn.WebParts.ashx');
    ...
    final body = {
      ...
      "Key": "d0ea794f",
      "GameDrawId": "",
      "ArrayNumbers": List.generate(5, (_) => List.generate(35, (_) => "")),
      "CheckMulti": false,
      "PageIndex": pageIndex
    };
    ...
    try {
      ...
      print('Error fetching page $pageIndex for product 535: $e');
      return [];
    }
  }
  ...
  List<LotteryDrawModel> _parseHtml(String htmlContent) {
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      final int pipeIndex = spanTexts.indexOf('|');
      if (pipeIndex != -1) {
        // Numbers before '|' are regular
        for (int j = 0; j < pipeIndex; j++) {
          final int? val = int.tryParse(spanTexts[j]);
          if (val != null) {
            regularNumbers.add(val);
          }
        }
        // Numbers after '|' are special
        for (int j = pipeIndex + 1; j < spanTexts.length; j++) {
          final int? val = int.tryParse(spanTexts[j]);
          if (val != null) {
        ...
        // Fallback: assume first 5 are regular and the rest are special
        final List<int> allNums = spanTexts
        ...
        if (allNums.length >= 6) {
          regularNumbers.addAll(allNums.sublist(0, 5));
          specialNumbers.addAll(allNums.sublist(5));
        } else {
